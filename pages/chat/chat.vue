<template>
	<view>
		...
		<chat-input @submit="send"></chat-input>
	</view>
</template>
	...
	export default {
		...
		onLoad() {
			...
			uni.getSystemInfo({
				success: res => {
					// console.log(res)
					// console.log('temp_userinfo',uni.getStorageSync("temp_userinfo"))
		...
		computed: {
			...
			scrollHeight(){
				return this.windowHeight -this.keyboardHeight-  uni.upx2px(120)
			}
		},
		beforeDestroy() {
			this.clear_chatList()
		...
		watch:{
			//监听接收消息
			receive(newVal){
				console.log('接收消息--',this.chatList)
				this.scrollToBottom()
			}
		},
		methods: {
