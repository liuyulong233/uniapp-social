...
<script>
	...
	export default {
		...
		onLoad({
			...
			const localPackageInfo = JSON.parse(PACKAGE_INFO);
			if (!localPackageInfo) {
		...
		computed: {
			...
			isAppStore() {
				// console.log('this.isiOS',this.isiOS,this.isWGT)
				return this.isiOS || (!this.isiOS && this.url.indexOf('.apk') === -1)
			}
		},
		methods: {
			//本地是否有下载好的安装包
			checkLocalStoragePackage() {
				...
				if (localFilePathRecord) {
					...
					// 比对版本
					if (!installed && compareVersion(version, this.version) === 0) {
